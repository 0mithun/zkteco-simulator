...
const generateUsers = (count, startUid) => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    users.set(uid, {
      ...
      password: '123456',
      name: fullName,
      cardno: Math.floor(100000 + Math.random() * 900000), // Random 6-digit card number
      userId: uid.toString()
    });
  }
...
const generateAttendanceRecord = (daysToGenerate = 180)=> {
  let snCounter = 1;
  const now = new Date();

  users.forEach(user => {
    for (let i = 0; i < daysToGenerate; i++) {
      // Target date: "i" days ago
      const targetDate = new Date();
      targetDate.setDate(now.getDate() - i);
      
      // Random records per day: Min 2, Max 5
      const recordsPerDay = Math.floor(Math.random() * (5 - 2 + 1)) + 2;

      for (let j = 0; j < recordsPerDay; j++) {
        // Create a random time within that specific day
        ...
        recordTime.setSeconds(Math.floor(Math.random() * 60));

        attendances.push({
          sn: snCounter++,
          userId: user.userId,
          userSn: user.uid, // Required for your buffer function
          timestamp: Math.floor(recordTime.getTime() / 1000), // Unix seconds
          type: Math.floor(Math.random() * 2), // Random 0 or 1
          state: Math.floor(Math.random() * 6) // Standard ZK states (0-5)
        });
      }
    }
  });
...
function encodeZkTime(time) {
  ...
  // The logic must be exactly this to match the library's decoder:
  // (Year*12 + Month-1)*31 + Day-1 turns into days, then into seconds
  const encoded = (
      ((year * 12 + month - 1) * 31 + day - 1) * 86400 + 
...
function createAttendanceRecord(attendance) {
  const record = Buffer.alloc(40);
  record.writeUInt16LE(attendance.userSn, 0);
  record.write(attendance.userId.padEnd(9, '\0'), 2, 9, 'ascii');
  record.writeUInt16LE(0, 11);

  // const timestamp = Math.floor(Date.now() / 1000);
  // const timestamp = encodeZkTime(attendance.timestamp)
  // record.writeUInt32LE(timestamp, 27);

  // Convert to milliseconds if input is in seconds
  const timestamp = attendance.timestamp < 10000000000 ? 
                    attendance.timestamp * 1000 : 
                    attendance.timestamp;

  const zkTime = encodeZkTime(timestamp);

  record.writeUInt32LE(zkTime, 27);
  record.writeUInt8(attendance.type, 26);
  record.writeUInt8(attendance.state, 31);
  record.fill(0, 32);
    ...
    try {
      ...
      switch (command) {
        ...
        case COMMANDS.CMD_GET_FREE_SIZES: {
          ...
          sizeData.writeUInt32LE(logCapacity, 56);           // dwLogCapacity
        ...
        case COMMANDS.CMD_DATA_WRRQ: {
          ...
          if (buffersEqual(data, REQUEST_DATA.GET_ATTENDANCE_LOGS)) {
            ...
            let offset = 4;
            for (const attendance of attendances) {
              const record = createAttendanceRecord(attendance);
              record.copy(allAttendanceData, offset);
              offset += 40;
            }
